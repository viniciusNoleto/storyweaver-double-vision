...
export function useApplyCharacterActionLogicData({ code, characterId }: { code: string; characterId: number | null }) {
  const applyCharacterActionMutation = useMutation({
    mutationFn: ({ type, amount }: { type: 'damage' | 'heal' | 'mana-spend' | 'mana-restore' | 'extra-add' | 'extra-remove'; amount: number }) => {
...
export function CharacterActionsPanel({
  ...
  const [extraAddAmount, setExtraAddAmount] = useState<number>(0);
  const [extraRemoveAmount, setExtraRemoveAmount] = useState<number>(0);
  ...
  function applyExtraAdd() {
    if (extraAddAmount <= 0) return;

    applyCharacterActionMutate({ type: 'extra-add', amount: extraAddAmount });
    setExtraAddAmount(0);
  }

  function applyExtraRemove() {
    if (extraRemoveAmount <= 0) return;

    applyCharacterActionMutate({ type: 'extra-remove', amount: extraRemoveAmount });
    setExtraRemoveAmount(0);
  }

  // Vida extra entra no cálculo (numerador e denominador) — mesma fórmula de
  // `HealthColor.ts`, usada aqui só para o `value` da barra de progresso (a
  // cor já vem de `healthColor()` abaixo).
  const percent = healthPercent(character.hp_current, character.hp_max, character.extra_hp);
  ...
  const isApplyingExtraAdd = applyCharacterActionIsPending && applyCharacterActionVariables?.type === 'extra-add';
  const isApplyingExtraRemove = applyCharacterActionIsPending && applyCharacterActionVariables?.type === 'extra-remove';
    ...
    >
      <div className="flex flex-col gap-4">
        <div>
          ...
          <Progress
            value={percent}
            color={healthColor(character.hp_current, character.hp_max, character.extra_hp)}
            size="lg"
            radius="xl"
          />
          ...
          >
            ...
            {character.extra_hp > 0 && (
              <Text
                size="lg"
                fw={700}
                className="mt-1 font-display text-[#4fce7f]"
              >
                {`+${character.extra_hp}`}
              </Text>
            )}
        ...
        <Divider label="Vida Extra" labelPosition="left" />
        ...
        >
          <NumberInput
            min={0}
            value={extraAddAmount}
            onChange={(value) => setExtraAddAmount(toNumber(value))}
            className="flex-1"
          />

          <Button
            color="green"
            loading={isApplyingExtraAdd}
            disabled={extraAddAmount <= 0}
            onClick={applyExtraAdd}
          >
        ...
        >
          <NumberInput
            min={0}
            value={extraRemoveAmount}
            onChange={(value) => setExtraRemoveAmount(toNumber(value))}
            className="flex-1"
          />
          ...
          <Button
            variant="outline"
            color="green"
            loading={isApplyingExtraRemove}
            disabled={extraRemoveAmount <= 0}
            onClick={applyExtraRemove}
          >
            Remover vida extra
          </Button>
        </Group>
