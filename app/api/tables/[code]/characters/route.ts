...
export async function POST(request: Request, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    // Personagem novo sempre cai na zona de menor `position` da Mesa
    // (primeira divisão) — mover entre zonas é feito depois via
    // `PATCH .../characters/[id]` com `zone_id`.
    ...
    if (!firstZone) {
      return NextResponse.json({ success: false, message: { 'pt-br': 'A Mesa não tem nenhuma divisão.', 'es-mx': 'La Mesa no tiene ninguna división.', 'en-us': 'The Table has no zones.' }, data: null }, { status: 422 });
    }
