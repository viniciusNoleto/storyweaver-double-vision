...
// Remove uma divisão/zona do tabuleiro. Só o Mestre pode. Todo o trabalho
// (reassociar personagens + deletar a zona + renumerar as restantes) roda
// numa única transação para nunca deixar `characters.zone_id` apontando para
// uma zona que não existe mais, nem `table_zones.position` com furos.
export async function DELETE(_request: Request, { params }: { params: Promise<{ code: string; id: string }> }) {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      // mantém a Mesa sem furos de `position`. `sequential` porque cada
      // update depende da mesma tabela, dentro da transação.
